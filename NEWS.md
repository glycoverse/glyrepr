...
* Core structure inspection, transformation, graph-table, composition, and IUPAC APIs now accept individual glycan `igraph` objects directly while preserving existing `glyrepr_structure` vector behavior; graph transformations and tables retain current vertex IDs. (#81)
