...
* `as_glycan_structure()` and `glycan_structure()` now support floating substituents with unresolved parent residues using `{<substituent>}` and `{<substituent>|<parents>}` syntax, including unknown carbon positions such as `{?S}`.
