...
`glyrepr` writes a less common configuration by concatenating `D` or `L`
before the residue name: `DFuc`, `LGul`, or `DFucf` for D-fucofuranose.
The unprefixed natural names remain canonical; redundant forms such as
`LFuc` and `DGul` are not accepted.
