...
IUPAC-condensed names normally omit absolute configuration and assume
the natural form, such as L-fucose for `Fuc` and D-gulose for `Gul`.
`glyrepr` writes a less common configuration with `D-` or `L-` before
the residue name: `D-Fuc`, `L-Gul`, or `D-Fucf` for D-fucofuranose. The
unprefixed natural names remain canonical; redundant forms such as
`L-Fuc` and `D-Gul` are not accepted.
...
An alditol is marked by adding `-ol` to the reducing-end residue while
retaining the reducing-end annotation used by `glyrepr`, for example
`Gal(b1-4)GlcNAc-ol(a1-`.

## Floating Parts

Sometimes a substructure is known but its parent in the complete glycan
is not. For example, a bi-antennary N-glycan may contain one sialic acid
...
Omitting a parent list means that every feasible node outside the
floating part’s own component is a candidate. To retain a known
candidate set, add a pipe followed by comma-separated complete-sequence
node indices:

    {Neu5Ac(a2-3)|2,5}<main glycan>

The indices follow the left-to-right residue order of the complete
IUPAC-condensed sequence: residues inside floating blocks are counted
before the main glycan, while substituent blocks contribute no nodes.
Thus the floating Neu5Ac above is node 1 and its candidate main-tree
residues are nodes 2 and 5. The `|2,5` suffix is a `glyrepr` extension
to curly-brace IUPAC notation. A floating part may target a residue in
another floating component, but not one in its own component. All
selected component attachments must be acyclic and ultimately connect to
the main tree.

For example, in `{Gal(b1-4)|2,3}Glc(a1-3)Man(a1-`, Gal is node 1 and may
attach to Glc (node 2) or Man (node 3).

An explicit candidate set needs at least two nodes to remain floating.
Singleton syntax is still accepted, but it fully localizes the
attachment and is serialized as an ordinary linkage. For example,
`{Neu5Ac(a2-3)|2}Gal(b1-4)GlcNAc(b1-` becomes
`Neu5Ac(a2-3)Gal(b1-4)GlcNAc(b1-`. Singleton localization is iterative,
so attaching one floating component to another can resolve further
domains.
...
while preserving duplicate floating parts and remapping every
candidate-parent set to the resulting complete-sequence order. Floating
substituents use the same global node domain and may therefore target
residues in floating parts.
...
#> [37] glue_1.8.1        ragg_1.5.2        rmarkdown_2.32    purrr_1.2.2      
