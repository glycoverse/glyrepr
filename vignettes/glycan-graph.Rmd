...
- `parents`: candidate vertex indices in the main tree. `integer()` means all
  feasible main-tree nodes. In a canonical structure, an explicit vector has at
...
as `"6S"` or `"?S"`, and `parents`. Empty `parents` means every feasible main
node; otherwise the values are global graph vertex indices.
