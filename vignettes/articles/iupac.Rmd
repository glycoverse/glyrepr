---
title: "IUPAC-Condensed Glycan Notation"
---
...
This article introduces IUPAC-condensed glycan notation,
the main text format used by `glyrepr` for representing glycan structures.
The goal is to show how a branched glycan structure can be written as a compact,
readable string.
...
Some formats are easy for people to read,
some are better for databases and software,
and others preserve detailed chemical information.
...
When building `glyrepr`,
we needed a native text format for glycan structures.
IUPAC-condensed is a practical choice because it balances readability and information content.
...
For other formats such as WURCS or GlycoCT,
the broader `glycoverse` workflow can use parser packages such as `glyparse`.
...
and IUPAC notation gives each one a short abbreviation:
...
For a broader list of symbols,
see the [SNFG website](https://www.ncbi.nlm.nih.gov/glycans/snfg.html) or run `available_monosaccharides()`.

IUPAC-condensed names normally omit absolute configuration and assume the
natural form, such as L-fucose for `Fuc` and D-gulose for `Gul`.
`glyrepr` writes a less common configuration with `D-` or `L-` before the
residue name: `D-Fuc`, `L-Gul`, or `D-Fucf` for D-fucofuranose. The unprefixed
natural names remain canonical; redundant forms such as `L-Fuc` and `D-Gul`
are not accepted.

### Step 2: Substituents

Glycans can contain chemical modifications called **substituents**.
In IUPAC-condensed notation,
substituents are written after the monosaccharide name.
...
Sometimes not all linkage details are known,
...
The main challenge in IUPAC-condensed notation is writing a branched tree as a linear string.
Branches are placed in square brackets and inserted before the residue they attach to.
...
This example has two chains of equal length.
The tie-breaker rule decides which one becomes the main chain.
...
it has three branches with the same length, branching on different residues.
In this case, we look for the first breaking point from right to left: the b4 Man.
Two mannoses are connected to that Man,
one with an a1-3 linkage and the other with an a1-6 linkage.
According to the tie-breaker rule,
we choose the a1-3 branch as the main chain.
...
Using the tie-breaker rule again,
the branch can be written as:
...
Sometimes a substructure is known but its parent in the complete glycan is not.
For example, a bi-antennary N-glycan may contain one sialic acid without enough
evidence to determine which terminal galactose carries it.
...
Omitting a parent list means that every feasible node outside the floating
part's own component is a candidate. To retain a known candidate set, add a
pipe followed by comma-separated complete-sequence node indices:
...
IUPAC-condensed sequence: residues inside floating blocks are counted before
the main glycan, while substituent blocks contribute no nodes. Thus the
floating Neu5Ac above is node 1 and its candidate main-tree residues are nodes
2 and 5. The `|2,5` suffix is a `glyrepr` extension to curly-brace IUPAC
notation. A floating part may target a residue in another floating component,
but not one in its own component. All selected component attachments must be
acyclic and ultimately connect to the main tree.

For example, in
`{Gal(b1-4)|2,3}Glc(a1-3)Man(a1-`, Gal is node 1 and may attach to
Glc (node 2) or Man (node 3).

An explicit candidate set needs at least two nodes to remain floating.
Singleton syntax is still accepted, but it fully localizes the attachment and
is serialized as an ordinary linkage. For example,
`{Neu5Ac(a2-3)|2}Gal(b1-4)GlcNAc(b1-` becomes
`Neu5Ac(a2-3)Gal(b1-4)GlcNAc(b1-`. Singleton localization is iterative, so
attaching one floating component to another can resolve further domains.
...
Floating blocks may contain more than one residue, and multiple blocks may
precede the same main glycan. `glyrepr` canonicalizes their order while
preserving duplicate floating parts and remapping every candidate-parent set
to the resulting complete-sequence order. Floating substituents use the same
global node domain and may therefore target residues in floating parts.
...
IUPAC-condensed notation is compact enough for routine analysis
while still preserving the structural details that `glyrepr` needs.
