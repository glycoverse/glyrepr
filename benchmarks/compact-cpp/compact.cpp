...
std::string residue_label(const Tree& t, int n) {
...
std::string int_join(const std::vector<int>& x) {
...
std::vector<int> positions(const std::string& s) {
...
std::string normalize_sub(std::string s) {
...
bool matching(std::vector<std::vector<int>> domains) {
...
std::string collapse_subs(std::vector<std::string> tokens) {
...
std::pair<std::string,std::string> extract_basic(std::string mono) {
...
std::pair<std::string,std::string> extract_residue(const std::string& mono,
...
void cache_tree(Tree& t, Function& order, bool byte_order) {
...
std::vector<int> candidates(const Part& p, int n) {
...
void resolve_singletons(Forest& f) {
...
std::vector<int> symmetry_labels(Forest& f, Function& bliss) {
...
List finish_forest(Forest& f, Function& order, bool byte_order, Function& bliss) {
  Tree& t=f.tree;
...
// [[Rcpp::export]]
List compact_parse(CharacterVector strings,CharacterVector residues,IntegerVector anomer_positions,
  ...
  for(int i=0;i<strings.size();++i) {
    ...
    try {out[i]=parse_complete(as<std::string>(strings[i]),known,configurations,order,byte_order,bliss);}
