result_dir <- file.path("benchmarks", "compact-cpp", "results-v2")
...
workload_coverage <- do.call(
  ...
  lapply(names(workloads), function(name) {
    ...
    data.frame(
      ...
      previous_fast = sum(coverage$status[ids] == "ok"),
...
paths <- c(
  ...
  file.path(
    result_dir,
    c("corpus.rds", "workloads.rds", "random-trees.rds", "extended-inputs.rds")
  )
)
write.csv(
