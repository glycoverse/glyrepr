...
result_dir <- file.path(compact_dir, "results")
x <- readRDS(file.path(result_dir, "corpus.rds"))
coverage <- read.csv(file.path(result_dir, "coverage.csv"))
set.seed(20260921)
ids <- list(
  fast_500 = sample(which(coverage$status == "ok"), 500),
  ordinary_500 = sample(which(!grepl("{", x, fixed = TRUE)), 500),
  mixed_500 = sample(seq_along(x), 500)
)
workloads <- lapply(ids, function(i) x[i])
workloads$duplicates_10000 <- rep(workloads$fast_500[1:100], 100)
if (worker == 1L) {
...
for (name in names(workloads)) {
  ...
  invisible(compact_structure(input[1:3]))
  for (round in 1:2) {
    ...
    for (method in methods) {
      record(
        ...
        if (method == "R") {
          ...
          function() compact_structure(input)
        }
      )
    }
  }
