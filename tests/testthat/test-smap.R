  ...
  add_names_regular <- function(g) {
    if (!("name" %in% igraph::vertex_attr_names(g))) {
      igraph::set_vertex_attr(
        g,
        "name",
        value = paste0("v", seq_len(igraph::vcount(g)))
      )
    } else {
      g
    }
    ...
    ~ {
      if (!("name" %in% igraph::vertex_attr_names(.x))) {
        igraph::set_vertex_attr(
          .x,
          "name",
          value = paste0("v", seq_len(igraph::vcount(.x)))
        )
      } else {
        .x
      }
...
test_that("smap2 functions work with regular functions", {
  ...
  n_vertices <- c(1, 2, 1)
  result_int <- smap2_int(structures, n_vertices, function(g, n) {
    igraph::vcount(g) + n
  })
  ...
  thresholds <- c(5, 6, 5)
  result_lgl <- smap2_lgl(structures, thresholds, function(g, t) {
    igraph::vcount(g) > t
  })
  expect_equal(length(result_lgl), 3)
  expect_type(result_lgl, "logical")
...
test_that("smap2 functions work with purrr-style lambda functions", {
  ...
  result_regular <- smap2_dbl(structures, weights, function(g, w) {
    igraph::vcount(g) * w
  })
  expect_equal(result_lambda, result_regular)
  ...
  thresholds <- c(5, 6, 5)
  result_lgl_lambda <- smap2_lgl(
    structures,
    thresholds,
    ~ igraph::vcount(.x) > .y
  )
  result_lgl_regular <- smap2_lgl(structures, thresholds, function(g, t) {
    igraph::vcount(g) > t
  })
  expect_equal(result_lgl_lambda, result_lgl_regular)
})
...
test_that("smap2 functions validate inputs", {
  ...
  expect_error(
    smap2_structure("not_a_structure", c(1), ~.x),
    "glycan_structure"
  )
...
test_that("spmap functions work with regular functions", {
  ...
  result <- spmap_dbl(list(structures, weights, factors), function(g, w, f) {
    igraph::vcount(g) * w * f
  })
  expect_equal(length(result), 2)
  expect_type(result, "double")
  ...
  add_values <- c(1, 2)
  result_int <- spmap_int(list(structures, add_values), function(g, a) {
    igraph::vcount(g) + a
  })
  ...
  thresholds <- c(5, 6)
  result_lgl <- spmap_lgl(list(structures, thresholds), function(g, t) {
    igraph::vcount(g) > t
  })
...
test_that("spmap functions work with purrr-style lambda functions", {
  ...
  result_lambda <- spmap_dbl(
    list(structures, weights, factors),
    ~ igraph::vcount(..1) * ..2 * ..3
  )
  result_regular <- spmap_dbl(
    list(structures, weights, factors),
  ...
  result_chr_lambda <- spmap_chr(
    list(structures, prefixes),
...
test_that("spmap functions work with recycling", {
  ...
  single_factor <- 3
  result <- spmap_dbl(
    list(structures, single_weight, single_factor),
    ~ igraph::vcount(..1) * ..2 * ..3
  )
  expect_equal(length(result), 3)
  expect_type(result, "double")
...
test_that("spmap functions validate inputs", {
  ...
  expect_error(
    spmap_dbl(list(c(1, 2, 3), c(1, 2, 3)), ~ .x * .y),
    "glycan_structure"
  )
  expect_error(
    spmap_structure(list("not_a_structure"), ~.x),
    "glycan_structure"
  )
...
test_that("spmap functions handle edge cases", {
  ...
  # Test with empty structures
  empty <- glycan_structure()
  empty_result <- spmap_dbl(
    list(empty, numeric(0), numeric(0)),
    ~ igraph::vcount(..1) * ..2 * ..3
  )
  expect_equal(length(empty_result), 0)
  expect_type(empty_result, "double")
...
test_that("simap functions work with regular functions", {
  ...
  result <- simap_chr(structures, function(g, i) {
    paste0("Structure_", i, "_vcount_", igraph::vcount(g))
  })
  expect_equal(length(result), 3)
  expect_type(result, "character")
...
test_that("simap functions work with purrr-style lambda functions", {
  ...
  result_lambda <- simap_chr(
    structures,
    ~ paste0("Pos", .y, "_vertices", igraph::vcount(.x))
  )
  result_regular <- simap_chr(structures, function(g, i) {
    paste0("Pos", i, "_vertices", igraph::vcount(g))
  })
  expect_equal(result_lambda, result_regular)
...
test_that("simap functions handle edge cases", {
  ...
  result <- simap_chr(
    single,
    ~ paste0("Index", .y, "_vertices", igraph::vcount(.x))
  )
  expect_equal(length(result), 1)
  expect_type(result, "character")
...
test_that("spmap_structure correctly updates unique structures count when modifications create duplicates", {
  ...
  result <- spmap_structure(
    list(structures, values1, values2),
    remove_linkages_func
  )
...
test_that("smap2 functions preserve names", {
  ...
  result_list <- smap2(structures, weights, function(g, w) {
    list(count = igraph::vcount(g), weight = w)
  })
  expect_equal(names(result_list), c("A", "B", "C"))
})
...
test_that("spmap functions preserve names", {
  ...
  result_list <- spmap(list(structures, weights, factors), function(g, w, f) {
    list(count = igraph::vcount(g), weight = w)
  })
...
test_that("spmap handles NA elements", {
  ...
  result_lambda <- spmap_dbl(list(structs, c(1, 2, 3)), function(g, n) {
    igraph::vcount(g) + n
  })
  expect_equal(result, result_lambda)
})
